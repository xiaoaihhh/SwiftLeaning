...
struct PropertiesTest: Runable {
    ...
    static func lazyStoredProperties() {
        print("\n-----------------属性：延迟加载属性测试----------------")
        class DataImporter {
...
//        }
    ...
    static func propertyObservers() {
        print("\n-----------------属性：属性观察器测试----------------")
        class Point {
            init(x: Double, y:Double) {
                // 在自身构造器中初始化属性时候不会触发属性观察器，但如果是类，在父类中如果初始化完成属性，子类构造器中在修改属性会触发属性观察器
                self.x = x
                self.y = y
            }
            // 声明属性观察器时候不影响设置默认值
        ...
        func swapTwoDouble(x: inout Double, y: inout Double) {
            (y, x) = (x, y)
        }
        let point1 = Point(x: 100, y: 200)
        // 将带有观察器的属性通过 in-out 方式传入函数，willSet 和 didSet 也会调用。这是因为 in-out 参数采用了拷入拷出(copy-in copy-out)内存模式：即在函数内部使用的是参数的 copy，函数结束后，又对参数重新赋值。
        swapTwoDouble(x: &point1.x, y: &point1.y)
        print("---------inout类型参数会触发属性观察器\n")
    }
    ...
    static func globalAndLocalVariables() {
        print("\n-----------------属性：全局和局部变量测试----------------")
